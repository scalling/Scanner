package com.zm.scanner.translator;
...
//                pixels[width * centerY + j] = PX_BLACK;
        ...
        if (right - left < width * 0.3f) {
            if (imageView != null) {
                ...
                bmp.recycle();
            BGAQRCodeUtil.d("初步判断无手机号码");
            return null;
        }
        BGAQRCodeUtil.d("初步判断可能存在手机号码");
...
//                    charMaxWidth = charMaxHeight;
        ...
        right = textRect[2];
        bottom = textRect[3];
        BGAQRCodeUtil.d("捕捉到到 " + charCount + " 个字符 ");
       // if (bottom - top > (right - left) / 5 || bottom - top == 0 || charCount != 11) {
        if (bottom - top > (right - left) / 5 || bottom - top == 0) {
            if (imageView != null) {
                bmp.setPixels(pixels, 0, width, 0, 0, width, height);
                //将裁切的图片显示出来
                showImage(bmp);
    ...
    public String filterRule() {
        return "^((13[0-9])|(15[^4])|(166)|(17[0-8])|(18[0-9])|(19[8-9])|(147,145))\\d{8}$";
    }
